    ...
    _responseHandler: function (requestParamsJSONCopy, req, userOptions, callback) {
        ...
        function processResponseType(res, responseString, callback) {
            ...
            if (RESPONSE_CONTENT_TYPE.indexOf(res.headers['content-type'].toLowerCase()) > -1) {
                ...
                parser.parseString(responseString, function (err, obj) {
        ...
        return function (res) {
            ...
            try {
                ...
                debug('Failed to parse statusCode as statusCode not provided in the response. ', responseString);
            }
            var charset = '';
            var content_type = '';
            ...
            res.on('end', function () {
                ...
                if (charset) {
                    try {
                        responseString = iconv.decode(bufferString, 'utf8').toString();
                    } catch (Exception) {
                ...
                if (statusCode > 399) {
                    return callback.call(self, new Error.AmazonMwsAPIError({
                        message: responseString || 'Error occured from AmazonMwsAPI',
                        detail: response,
                    ...
                    processResponseType(res, responseString, function (error, response) {
                        ...
                        if (error) {
                            ...
                            return callback.call(self, new Error.AmazonMwsAPIError({
                                message: 'Failed to parse response received from the AmazonMws API',
                                detail: response,
                                exception: error || responseString,
                                StatusCode: statusCode || 'unknown'
                            }), null);
                        }
