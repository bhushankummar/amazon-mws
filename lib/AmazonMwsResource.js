...
AmazonMwsResource.prototype = {
        ...
        return function (res) {
            ...
            res.on('end', function () {
                try {
                    ...
                    processResponseType(res, response, function (error, response) {
    ...
    _request: function (method, path, data, auth, options, callback) {
        ...
        self.body = {};
        if (method === 'POST') {
            self.body.FeedContent = _.clone(self.requestParams.FeedContent);
            ...
            var stringToSign = ['POST', self._AmazonMws.getApiField('host'), path, qs.stringify(sorted)].join('\n');
        ...
        self.requestBodyJSON = _.clone(self.requestParams);
        if (self.requestDataProcessor) {
            self.requestParams = self.requestDataProcessor(method, data, options.headers);
        } else {
        ...
        if (self.requestBodyJSON.Action === 'SubmitFeed') {
            headers['Content-Type'] = 'x-www-form-urlencoded';
            headers['Content-MD5'] = self.requestBodyJSON.ContentMD5Value;
        }
